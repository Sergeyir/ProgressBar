...
ProgressBar::ProgressBar(std::string custom_left_border, const char custom_complete, 
                        const char custom_next_complete, const char custom_not_complete,
                        std::string custom_right_border, std::string color, 
                        std::string left_text, const int default_width)
{
...
void ProgressBar::SetCustomStyle(std::string custom_left_border, const char custom_complete, 
                                 const char custom_next_complete, const char custom_not_complete,
                                 std::string custom_right_border, std::string color)
{
...
void ProgressBar::SetWidth(const int default_width)
{
...
void ProgressBar::SetText(std::string left_text)
{
   text = left_text;
...
{   
   ...
   std::string progress_perc = DtoStr(
      (bar_progress + bar_step) * 100.0, bar_precision);
   
   ioctl(STDOUT_FILENO, TIOCGWINSZ, &w);
   
   int width;
   if (default_bar_width > w.ws_col - 4) 
   {
      width = w.ws_col - 4 - orig_default_width + 
      default_bar_width - utf8_strlen(progress_perc);
   }
...
void ProgressBar::Reset() {bar_progress = 0;}

void ProgressBar::SetBarStep(const double step) {bar_step = step;}

void ProgressBar::SetPrecision(const int precision) {bar_precision = precision;}

ProgressBar::~ProgressBar() {};

int ProgressBar::utf8_strlen(const std::string& str)
{
   int len = 0;
   for (long unsigned int i = 0; i < str.length(); i++, len++)
   {
...
std::string ProgressBar::CheckStyle(std::string style)
{
...
std::string ProgressBar::DtoStr(const double val, const int precision)
{
